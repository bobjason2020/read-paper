...
4. **步骤3.5：逐张精读图片并填充解读文档（🔴核心步骤）**
5. 步骤4：深度分析论文内容（阅读PDF+TeX）
6. 步骤5：生成完整精读笔记（插入已完成的图片解读）
7. 步骤6：更新阅读状态
8. 步骤7：输出操作反馈
```
...
- 🚫 **禁止**跳过步骤2（图片判断）或步骤4（图片精读）
- 🚫 **禁止**在所有图片解读完成前阅读论文正文（步骤4必须在步骤3.5之后）
...
### 步骤3.5：逐张精读图片并填充解读文档 ⭐核心步骤

**⚠️ 警告：此步骤不可跳过！必须逐张视觉分析所有选中图片！**

**本步骤必须完全在步骤4（读论文正文）之前完成，确保对每张图片都有深入理解后再开始分析论文全文。**

**工作流程：**

对 `images/` 目录下的每张选中图片，逐一执行以下操作：

1. **使用 `Read` 工具读取图片**，仔细观察图片内容
2. **读取对应的 `xxx.md` 解读文档模板**
3. **填充解读文档**，包括：

| 字段 | 填写内容 | 示例 |
|------|----------|------|
| **图片类型** | 架构图/流程图/实验结果/消融实验/对比图等 | 架构图 |
| **简要描述** | 一句话概括图片核心信息 | Transformer整体架构，包含编码器、解码器和注意力机制 |
| **组件说明** | 图片中各部件/区域的含义 | Multi-Head Attention: 8头并行注意力；FFN: 前馈神经网络 |
| **数据解读** | 图片中数据的含义（如果有） | 横轴为训练步数，纵轴为BLEU分数，虚线为基线方法 |
| **关键洞察** | 图片揭示的核心发现或结论 | 注意力机制能有效捕捉长距离依赖，显著优于RNN |

**示例（填充后内容）：**
```markdown
# transformer-architecture.png 图片解读

## 图片类型
架构图

## 简要描述
Transformer模型整体架构，包含编码器、解码器、注意力机制和位置编码。

## 组件说明
- **Input Embedding**: 输入词嵌入 + 位置编码
- **Encoder Stack**: 6层编码器，每层包含多头自注意力和前馈网络
- **Decoder Stack**: 6层解码器，额外包含交叉注意力层
- **Output**: 线性层 + Softmax 输出概率分布
...
## 关键洞察
架构设计简洁优雅，通过并行注意力机制替代RNN的序列化计算，实现了训练效率的大幅提升。
```

**执行顺序：**
- 🔴 **高优先级图片** → 优先精读，必须完全填充后再处理下一张
- 🟡 **中优先级图片** → 其次处理，同样需要完整填充
- 处理完所有图片后，才可进入步骤4
...
| **关键图表解读** | 引用步骤3.5已完成的 `images/xxx.md` 解读文档 | 汇总章节，直接引用已有文档 |
...
6. ❌ **禁止在步骤3.5完成前阅读论文正文** - 必须先精读所有图片再分析正文
